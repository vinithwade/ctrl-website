...
const SpaceBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const createStars = () => {
      stars.length = 0;
      const starCount = Math.floor(canvas.width * canvas.height / 1200); // Increased density
      
      for (let i = 0; i < starCount; i++) {
        stars.push({
          x: Math.random() * canvas.width,
          y: Math.random() * canvas.height,
          radius: Math.random() * 2.5, // Larger stars
          opacity: Math.random() * 0.8 + 0.4, // Between 0.4 and 1.2 (more visible)
    ...
    const createNebulae = () => {
      nebulae.length = 0;
      const nebulaCount = 4 + Math.floor(canvas.width / 400); // 4-8 nebulae depending on screen size
      ...
      for (let i = 0; i < nebulaCount; i++) {
        const opacity = 0.15 + Math.random() * 0.15; // Increased opacity (0.15-0.30)
        ...
        nebulae.push({
          x: Math.random() * canvas.width,
          y: Math.random() * canvas.height,
          radius: 200 + Math.random() * 350, // Larger nebulae
          color: color,
          opacity: opacity
        });
      }
    ...
    const animate = () => {
      ...
      gradient.addColorStop(0, 'rgba(5, 5, 15, 1)'); // Slightly lighter
      gradient.addColorStop(1, 'rgba(2, 2, 20, 1)'); // Slightly more contrast
      ...
      if (Math.random() < 0.01) { // Increased chance
        ...
        const length = 80 + Math.random() * 120; // Longer shooting stars
        ...
        shootingStar.addColorStop(0.5, 'rgba(255, 255, 255, 0.9)'); // Brighter
